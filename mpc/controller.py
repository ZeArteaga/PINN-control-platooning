...
def setupDMPC(model: Model, config: dict, get_prec_state) -> MPC:
    ...
    e_i_expr = model.aux['e_i']
    u_i = model.u['u_i']

    Q = config['Q'] #single value in this case
    R = config['R'] #this too
    lterm = Q * e_i_expr**2  #*LAGRANGE/ERROR PENALTY
    mterm = e_i_expr**2 #*TERMINAL COST 
    mpc.set_objective(lterm=lterm, mterm=mterm) #, mterm=mterm)
    mpc.set_rterm(u_i=R) #*ACTUATOR PENALTY
    ...
    mpc.bounds['lower', '_u', 'u_i'] = u_min
    mpc.bounds['upper', '_u', 'u_i'] = u_max

    v_min = config.get('v_min', 0.0)
    v_max = config.get('v_max', 120/3.6) # e.g., max velocity
    mpc.bounds['lower', '_x', 'v_i'] = v_min
    mpc.bounds['upper', '_x', 'v_i'] = v_max
    
    #position has no bounds i guess but spacing:
 
    #try to do this instead later, instead of specifying d_min in objective
...
def setupSim(model: Model, t_step, get_prec_state) -> Simulator:
    ...
    def tvp_fun(t_now): #create time varying parameter fetch function
        x, v = get_prec_state(t_now) #*get preceeding vehicle state...
        tvp_template['t'] = t_now 
        tvp_template['x_prec'] = x #*though this time no prediciction is made since this sim model (plant)
        tvp_template['v_prec'] = v 
        return tvp_template
