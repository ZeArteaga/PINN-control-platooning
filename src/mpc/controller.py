...
def setupDMPC(model: Model, config: dict, opt_params: dict, get_prec_state, platoon: list, fv_idx: int) -> MPC:
     #*MPC----
    mpc = MPC(model)
...
def setupSim(model: Model, sim_config: dict, get_prec_state, platoon: list, fv_idx: int) -> Simulator:
    sim = Simulator(model)
